# io_scr.cpp
## English
### Purpose
Main functional areas: screen operations
### Key Functions
- `MakePathString()`: Performs MakePathString operation as part of the component's functionality
- `MakePatternStr()`: Performs MakePatternStr operation as part of the component's functionality
### Summary
The `io_scr.cpp` file provides essential functionality for screen operations. and contains approximately 2 function(s) to support the Far Manager file manager operations.

---

## Polski (Polish)
### Cel
Główne obszary funkcjonalne: operacje ekranowe
### Kluczowe Funkcje
- `MakePathString()`: Wykonuje MakePathString operację jako część operacje ekranowe
- `MakePatternStr()`: Wykonuje MakePatternStr operację jako część operacje ekranowe
### Podsumowanie
Plik `io_scr.cpp` zapewnia podstawową funkcjonalność dla operacje ekranowe. i zawiera około 2 funkcji wspierających operacje menedżera plików Far Manager.
